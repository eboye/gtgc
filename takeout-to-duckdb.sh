...
printf "Extracting semantic locations..."
echo "$FILES_IN_TAKEOUT" | while read -r FILE_IN_TAKEOUT; do
  if expr "$FILE_IN_TAKEOUT" : '[0-9]\{4\}/[0-9]\{4\}_[A-Z]\+\.json' > /dev/null; then
...
printf "Extracting location records..."
...
printf "Creating DuckDB database..."
...
printf "Optimizing DuckDB database..."
